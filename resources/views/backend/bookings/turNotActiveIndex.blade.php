...
@section('content')
  ...
  @if(session('status'))
    <div class="alert alert-success" role="alert">
      {{ session('status') }}... <a href="{{ url('/admin/sifarisler/turaktivsifarisler') }}">Burdan</a> aktiv sifarişlərə baxa bilərsiniz.
    </div>
  @endif

  @if(count($sifarisler) == 0)
    <div class="alert alert-warning" role="alert">
      Hələki saytda aktiv olmayan sifariş yoxdur..
    </div>
  @else
                ...
                <tbody>
                  @foreach($sifarisler as $sifaris)
                    ...
                    <tr>
                      ...
                      <td>
                        <a href="{{ url('/admin/sifaris/active/'.$sifaris->id) }}" class="settings" title="Aktive Et" data-toggle="tooltip" data-original-title="Aktiv Et"><i class="ti-check"></i></a>
                        <a href="{{ url('/admin/sifaris/sil/'.$sifaris->id) }}" class="delete" title="Sil" data-toggle="tooltip" data-original-title="Sil"><i class="ti-trash"></i></a>
                      </td>
                    </tr>
